...
export default function LotteryDraw({ draw }: { draw: Draw }) {
    ...
    return (
        <div className="flex flex-col bg-white/10 backdrop-blur-lg p-3 sm:p-4 rounded-lg shadow-lg m-2 w-full max-w-2xl">
            ...
            <span className="text-blue-500 ml-auto text-sm sm:text-base cursor-pointer" onClick={() => setShowDetail(!showDetail)}>{showDetail ? '收起' : '展开'}</span>
            {
                showDetail &&
                            ...
                            <tbody>
                                {prizegrades.slice(0, 3).map((prize) => (
                                    <tr key={prize.type}>
                                        ...
                                        <td className="p-2">{prize.typenum}</td>
                                        <td className="p-2">{prize.typemoney}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                    </div>
                    <p className="text-xs sm:text-sm mt-2">{`销售额:${draw.sales}`}</p>
                    <p className="text-xs sm:text-sm mt-2">{`奖池:${draw.poolmoney}`}</p>
                </div>
            }
        </div>
    );
